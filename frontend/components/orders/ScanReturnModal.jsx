...
export default function ScanReturnModal({ open, onClose, onReturned }) {
  ...
  async function onSubmit(e) {
    ...
    setSubmitting(true);
    try {
      const result = await ordersService.scanReturn({ orderNumber });
      setResults((r) => [{ ...result, order_number: result.order_number || orderNumber }, ...r]);
      if (result.success) onReturned?.();
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Scan and Return">
      <p className="mb-3 text-sm text-slate-500">
        Scan or type each order number, then press Enter. Only Dispatched/Delivered orders can be
        marked returned.
      </p>
      ...
      <ul className="mt-4 max-h-64 space-y-1 overflow-y-auto text-sm">
        {results.map((r, i) => (
          ...
          >
            ...
            <span className="text-xs">{r.success ? "Returned" : r.reason}</span>
          </li>
        ))}
      </ul>
    </Modal>
